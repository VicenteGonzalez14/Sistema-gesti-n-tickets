...
void list_pushBack(List *L, Node *nuevoTicket) {
  if (L == NULL) return;

  nuevoTicket->next = NULL; // Asegura que el nuevo nodo no apunte a otro

  printf("Agregando ticket con ID: %s\n", nuevoTicket->data);

  if (L->tail == NULL) { // Lista vacía
    L->head = nuevoTicket;
    L->tail = nuevoTicket;
  } else {
    L->tail->next = nuevoTicket;
    L->tail = nuevoTicket;
  }
  L->size++;
}
